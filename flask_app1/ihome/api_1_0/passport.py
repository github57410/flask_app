...
def login():
    ...
    # 4 手机号的格式
    if not re.match(r"1[34578]\d{9}", mobile):
        return jsonify(errno=RET.PARAMERR, errmsg="手机参数格式错误")

    # 5 判断错误次数是否超过限制  如果超过限制 则返回
    # redis 记录： “access_num_请求的ip”: 次数
    user_ip = request.remote_addr # 用户的ip地址
    try:
        accesss_nums = redis_store.get("access_nums_%s" % user_ip)
    except Exception as e:
        current_app.logger.error(e)
    else:
        if accesss_nums is not None and int(accesss_nums) >= constants.LOGIN_ERROR_MAX_TIMES:
            return jsonify(errno=RET.REQERR, errmsg="错误次数过多，请稍后重试")
    # 从数据库的密码与用户填写的密码进行对比验证
    try:
        user = User.query.filter_by(mobile=mobile).first()
    except Exception as e:
        current_app.logger.error(e)
